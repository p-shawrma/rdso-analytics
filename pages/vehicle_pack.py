...
# Function to create ClickHouse client
...
# Function to fetch all Model_Numb and DeviceDate for filters
@st.cache(allow_output_mutation=True, ttl=6000, show_spinner=False)
def fetch_all_model_date():
    client = create_clickhouse_client()
    query = "SELECT DISTINCT Model_Numb, DeviceDate FROM telematics.custom_tracking"
    result = client.query(query)
    df = pd.DataFrame(result.result_rows, columns=result.column_names)
    return df

# Function to fetch filtered data from ClickHouse based on selected Model_Numb and dates
@st.cache(allow_output_mutation=True, ttl=6000, show_spinner=False)
def fetch_filtered_data(model_numbers, start_date, end_date):
    client = create_clickhouse_client()
    model_numbers_str = ', '.join([f"'{model}'" for model in model_numbers])
    query = f"""
    SELECT *
    FROM telematics.custom_tracking
    WHERE Model_Numb IN ({model_numbers_str}) 
    AND DeviceDate BETWEEN '{start_date}' AND '{end_date}';
    """
    result = client.query(query)
    df = pd.DataFrame(result.result_rows, columns=result.column_names)
    return df
...
def plot_discharge_currents(df):
    ...
    discharge_df = df[df['step_type'] == 1]  # 1 represents 'discharge'
...
def create_day_wise_summary(df):
    ...
    discharge = df[df['step_type'] == 1]  # 1 represents 'discharge'
    charge = df[df['step_type'] == 0]  # 0 represents 'charge'
...
def main():
    ...
    with st.sidebar:
        ...
        filter_data = fetch_all_model_date()
        model_numbers = filter_data['Model_Numb'].unique().tolist()
        selected_models = st.multiselect('Select Model Numbers', model_numbers, default=model_numbers)
        dates = filter_data['DeviceDate'].unique()
    ...
    if fetch_button or 'data_loaded' not in st.session_state:
        ...
        df = fetch_filtered_data(selected_models, start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
        if not df.empty:
